function [mean_sensitivity]=plot_sensitivity_ranges(n, BM ,X,Y, names)
...
for i=1:n_x
    ...
    for j = 1: n 
        ...
        %Y_value(:,j) = Y_sensitivity(:,i);
        %plot(X_value(:,j), Y_value (:,j), 'k-','LineStyle','-','Linewidth',2); hold on
    end
    % Get confidence intervals
    [Y_range.low, Y_range.up] = confidence_intervals (Y_value');
    % Plot confidence interval
    P3 = Fill_Ranges(X.maxmin(:,i),Y_range.low,Y_range.up, [0.5 0.5 0.5]);hold on 
    % Plot Monte Carlomean value
    P4 = plot(X.maxmin(:,i),mean(Y_value,2),'r--','LineStyle','--','Linewidth',2);  hold on
    %P5 = plot(X_cal(:,i),Y_cal,'ko','MarkerSize',8);  hold on
    
    % Plot selected model
    P5 = plot(X.maxmin(:,i),Y(BM).Y_sensitivity(:,i),'k-','LineStyle','-','Linewidth',1.5);  hold on
    
    alpha(P3,.3);
    
    % Save mean value for output
    mean_sensitivity(:,i)=mean(Y_value,2);
    ...
    if i==1
        leg1 = legend([P3, P4, P5],['95 $\%$ uncertainty'], ...
            ['Ensemble mean'],['Selected model'],'location', ...
